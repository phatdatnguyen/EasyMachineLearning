...
{
    public partial class RandomForestModelForm : Form
    {
        //Fields
        ...
        //Constructor
        public RandomForestModelForm(DataTable inputData, string[] inputColumnNames, string outputColumnName)
        {
            InitializeComponent();
            ...
            //Tab learning
            numberOfTreesNumericUpDown.Maximum = inputData.Rows.Count / 2;
            ...
            pruningFromNumericUpDown.Maximum = inputData.Rows.Count;
            pruningToNumericUpDown.Maximum = inputData.Rows.Count;
            pruningToNumericUpDown.Value = inputData.Rows.Count;
        ...
        {
            int k = Convert.ToInt16(numberOfTreesNumericUpDown.Value);
            ...
            try
            {
                ...
                {
                    NumberOfTrees = (int)numberOfTreesNumericUpDown.Value,
                    SampleRatio = (double)sampleRatioNumericUpDown.Value
                };
                randomForest = randomForestLearning.Learn(inputColumns.Get(learningIndexes), outputGroupIndexes.Get(learningIndexes));
            }
            ...
            for (int treeIndex = 0; treeIndex < randomForest.Trees.Length; treeIndex++)
                treeComboBox.Items.Add(treeIndex + 1);
        ...
        private void pruningMethodComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (pruningMethodComboBox.SelectedIndex < 0)
                return;

            pruningThresholdNumericUpDown.Enabled = pruningMethodComboBox.SelectedItem.ToString() == "Error-Based Pruning";
        }

        private void pruneButton_Click(object sender, EventArgs e)
        {
            int fromRowIndex = Convert.ToInt16(pruningFromNumericUpDown.Value) - 1;
            int toRowIndex = Convert.ToInt16(pruningToNumericUpDown.Value) - 1;
            if (fromRowIndex > toRowIndex)
            {
                MessageBox.Show(this, "Invalid data range!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            int[] pruningIndexes = new int[toRowIndex - fromRowIndex + 1];
            for (int rowIndex = 0; rowIndex < toRowIndex - fromRowIndex + 1; rowIndex++)
                pruningIndexes[rowIndex] = fromRowIndex + rowIndex;
            ...
            Cursor = Cursors.WaitCursor;
            toolStripStatusLabel.Text = "Pruning...";

            try
            {
                foreach (DecisionTree decisionTree in randomForest.Trees)
                {
                    if (pruningMethodComboBox.SelectedItem.ToString() == "Error-Based Pruning")
                    {
                        ...
                        ErrorBasedPruning prune = new ErrorBasedPruning(decisionTree, inputColumns.Get(pruningIndexes), outputGroupIndexes.Get(pruningIndexes));

                        // Gain threshold
                        prune.Threshold = (double)pruningThresholdNumericUpDown.Value;
                    ...
                    {
                        // Now we can attempt to prune the tree using the pruning groups
                        ReducedErrorPruning prune = new ReducedErrorPruning(decisionTree, inputColumns.Get(pruningIndexes), outputGroupIndexes.Get(pruningIndexes));
            ...
            decisionTreeView.TreeSource = randomForest.Trees[treeComboBox.SelectedIndex];

            toolStripStatusLabel.Text = "Pruning completed!";
            Cursor = Cursors.Arrow;
        }
        ...
        private void CreateSurface(string xAxisColumnName, string yAxisColumnName)
        {
            ...
            int[] result = randomForest.Decide(map);
        ...
        {
            if (treeComboBox.SelectedIndex < 0)
                return;
        ...
        private void testButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                computedOutputsGroupIndexes = randomForest.Decide(inputColumns.Get(testingIndexes));
            }
            catch (Exception exception)
            {
                MessageBox.Show(this, exception.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void predictButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                outputGroupIndex = randomForest.Decide(inputValues);
            }
            catch (Exception exception)
            {
                MessageBox.Show(this, exception.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);

...
{
    public partial class DecisionTreeModelForm : Form
    {
        //Fields
        ...
        //Constructor
        public DecisionTreeModelForm(DataTable inputData, string[] inputColumnNames, string outputColumnName)
        {
            InitializeComponent();
            ...
            try
            {
                if (learningMethodComboBox.SelectedItem.ToString() == "C45")
                {
                    ...
                    for (int columnIndex = 0; columnIndex < inputColumnNames.Length; columnIndex++)
                        decisionVariables[columnIndex] = new DecisionVariable(inputColumnNames[columnIndex], DecisionVariableKind.Continuous);
                    C45Learning c45Learning = new C45Learning(decisionVariables);
                    decisionTree = c45Learning.Learn(inputColumns.Get(learningIndexes), outputGroupIndexes.Get(learningIndexes));
                }
                else //learningMethodComboBox.SelectedItem.ToString() == "ID3"
                {
                    DecisionVariable[] decisionVariables = new DecisionVariable[inputColumnNames.Length];
                    for (int columnIndex = 0; columnIndex < inputColumnNames.Length; columnIndex++)
                        decisionVariables[columnIndex] = new DecisionVariable(inputColumnNames[columnIndex], DecisionVariableKind.Discrete);
                    ID3Learning id3Learning = new ID3Learning(decisionVariables);
                    decisionTree = id3Learning.Learn(inputColumns.Get(learningIndexes).ToInt32(), outputGroupIndexes.Get(learningIndexes));
                }
            }
        ...
        private void CreateSurface(string xAxisColumnName, string yAxisColumnName)
        {
            ...
            int[] result = decisionTree.Decide(map);
        ...
        private void testButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                computedOutputsGroupIndexes = decisionTree.Decide(inputColumns.Get(testingIndexes));
            }
            catch (Exception exception)
            {
                MessageBox.Show(this, exception.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void predictButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                outputGroupIndex = decisionTree.Decide(inputValues);
            }
            catch (Exception exception)
            {
                MessageBox.Show(this, exception.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
